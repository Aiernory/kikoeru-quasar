const mutations = {
  ...
  SET_VOLUME_ALL: (state, val) => {
    ...
    if (state.volumeWork >= -9 && state.volumeWork <= 9 && state.volumeAll >= 0 && state.volumeAll <= 100) {
      if (state.volumeWork >= 0) {
        state.volume = state.volumeAll * (1 + state.volumeWork ) / 100
      } else {
        state.volume = state.volumeAll / (1 - state.volumeWork ) / 100
      }
